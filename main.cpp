...
int main(int argc, const char * argv[]) {
  ...
  save2obj("out.myfile.city.obj", j);

  //-- enrich with some attributes and save to a new CityJSON 
  enrich_and_save("out.myfile.city.json", j);
...
void save2obj(std::string filename, const json& j) {
  ...
  //-- fetch all the vertices in real-world coordinates (so "transform" is applied)$ g++ main.cpp
...
//  std::uniform_real_distribution<> dis(100.0, 200.0);
  ...
  for (auto& co : j["CityObjects"].items()) {    //.items() return the key-value pairs belong to the CityObjects
          if (co.value()["type"] == "BuildingPart") {
              ...
              //change here to return an area_list too for roughness index
